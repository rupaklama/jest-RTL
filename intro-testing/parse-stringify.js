// takes in query string '?by=kati-frantz' and returns { by: 'kati-frantz' }

const removeQueryChar = queryString => {
  return queryString.startsWith("?") ? queryString.slice(1).split("&") : queryString.split("&");
};
const parse = queryString => {
  ...
  let queries = removeQueryChar(queryString);
  const params = {};
